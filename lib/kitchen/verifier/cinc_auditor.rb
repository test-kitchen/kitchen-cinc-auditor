...
module Kitchen
  module Verifier
    # Cinc Auditor verifier for Kitchen.
    #
    # @author Fletcher Nichol <fnichol@chef.io>
    class CincAuditor < Kitchen::Verifier::Base # rubocop:disable Metrics/ClassLength
      kitchen_verifier_api_version 1
      plugin_version Kitchen::Verifier::CINC_AUDITOR_VERSION

      # Cinc Auditor is based on RSpec, which is not thread safe
      ...
      def finalize_config!(instance)
        ...
        # We want to switch kitchen-cinc-auditor to look for its tests in
        ...
        cinc_auditor_test_dir = File.join(config[:kitchen_root], "test", "recipes")
        if File.directory?(cinc_auditor_test_dir)
          config[:test_base_path] = cinc_auditor_test_dir
        end
      ...
      def call(state)
        ...
        cinc_auditor_config = ::Inspec::Config.new(opts)

        # handle plugins
        setup_plugin_config(cinc_auditor_config)

        # initialize runner
        runner = ::Inspec::Runner.new(cinc_auditor_config)
      ...
      def setup_waivers(opts, config)
        # Cinc Auditor expects the singular inflection
        opts[:waiver_file] = config[:waiver_files] || []
      end

      def setup_inputs(opts, config)
        cinc_auditor_version = Gem::Version.new(::Inspec::VERSION)

        # Handle input files
        if config[:attrs]
          logger.warn("kitchen-cinc-auditor: please use 'input-files' instead of 'attrs'")
          config[:input_files] = config[:attrs]
        end
        if config[:input_files]
          # Note that Cinc Auditor expects the singular inflection, input_file
          files_key = cinc_auditor_version >= Gem::Version.new("3.10") ? :input_file : :attrs
          opts[files_key] = config[:input_files]
        end

        # Handle YAML => Hash inputs
        if config[:attributes]
          logger.warn("kitchen-cinc-auditor: please use 'inputs' instead of 'attributes'")
          config[:inputs] = config[:attributes]
        end
        if config[:inputs]
          # Version here is dependent on upstream InSpec issue 3856
          inputs_key = cinc_auditor_version >= Gem::Version.new("4.10") ? :inputs : :attributes
      ...
      def setup_plugin_config(cinc_auditor_config)
        return unless config[:load_plugins]

        unless cinc_auditor_config.respond_to?(:merge_plugin_config)
          logger.warn("kitchen-cinc-auditor: skipping `plugin_config` which requires Cinc Auditor version 4.26.2 or higher. Your version: #{::Inspec::VERSION}")
          return
        end

        config[:plugin_config].each do |plugin_name, plugin_config|
          cinc_auditor_config.merge_plugin_config(plugin_name, plugin_config)
        end
      end
      ...
      # - test/integration
      # - test/integration/cinc-auditor (preferred if used with other test environments)
      #
      # we do not filter for specific directories, this is core of Cinc Auditor
      #
      ...
      def local_suite_files
        ...
        %w{cinc-auditor inspec serverspec bats pester rspec cucumber minitest bash}.each do |fw|
        ...
        if legacy_mode
          # Prefer cinc-auditor directory, but fall back to inspec for backward compatibility
          if Pathname.new(File.join(base, "cinc-auditor")).exist?
            base = File.join(base, "cinc-auditor")
          elsif Pathname.new(File.join(base, "inspec")).exist?
            base = File.join(base, "inspec")
          end
        end
      ...
      def resolve_config_inspec_tests
        config[:inspec_tests].map do |test_item|
          if test_item.is_a?(Hash)
            ...
            # We will let it fall through and let Cinc Auditor raise the appropriate exception.
      ...
      # Returns a configuration Hash that can be passed to an `Inspec::Runner`.
      #
      ...
      def runner_options(transport, state = {}, platform = nil, suite = nil) # rubocop:disable Metrics/AbcSize
        ...
        if respond_to?("runner_options_for_#{transport.name.downcase}", true)
          ...
          # default color to true to match Cinc Auditor behavior
          ...
          if config[:backend_cache]
            ...
            # default to false until we default to true in Cinc Auditor
