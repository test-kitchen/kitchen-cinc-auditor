...
def kitchen_cinc_auditor_version(target = nil)
  path = "lib/kitchen/verifier/cinc_auditor_version.rb"
  require_relative path.sub(/.rb$/, "")

  nu_version = target.nil? ? "" : " -> #{target}"
  puts "Kitchen-cinc-auditor: #{Kitchen::Verifier::CINC_AUDITOR_VERSION}#{nu_version}"

  unless target.nil?
    raw = File.read(path)
    nu = raw.sub(/CINC_AUDITOR_VERSION.*/, "CINC_AUDITOR_VERSION = \"#{target}\"")
    File.write(path, nu)
    load(path)
  end
end
...
task :version do
  kitchen_cinc_auditor_version
end

desc "Generate the changelog"
task :changelog do
  require_relative "lib/kitchen/verifier/cinc_auditor_version"
  system "github_changelog_generator -u test-kitchen -p kitchen-cinc-auditor --future-release #{Kitchen::Verifier::CINC_AUDITOR_VERSION}"
end
...
task :bump_version, [:version] do |_, args|
  ...
  kitchen_cinc_auditor_version(v)
