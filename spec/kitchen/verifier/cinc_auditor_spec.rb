...
describe Kitchen::Verifier::CincAuditor do
  ...
  let(:verifier) do
    Kitchen::Verifier::CincAuditor.new(config).finalize_config!(instance)
  end
  ...
  it "plugin_version is set to Kitchen::Verifier::CINC_AUDITOR_VERSION" do
    expect(verifier.diagnose_plugin[:version])
      .to eq(Kitchen::Verifier::CINC_AUDITOR_VERSION)
  end
  ...
  context "with an ssh transport" do
    ...
    it "constructs a Inspec::Runner with a specified output format" do
    ...
    it "constructs a Inspec::Runner with a controls filter" do
    ...
    it "does not send keys_only=true to CincAuditor (which breaks SSH Agent usage)" do
      expect(Inspec::Runner).to receive(:new)
        .with(
          hash_not_including(
            "keys_only" => true
