...
RSpec.describe Kitchen::Verifier::CincAuditor do
  ...
  context "with ssh transport" do
    ...
    it "lets the verifier forward_agent option win over the transport setting" do
      allow(transport).to receive(:connection_options).and_return(
        hostname: "host.example",
        port: 22,
        username: "cinc",
      ...
      expect(verifier.send(:runner_options, transport)).to include("forward_agent" => true)
    end
  ...
  context "with an unsupported transport" do
    ...
    it "raises a UserError naming the transport" do
      expect { verifier.call({}) }.to raise_error(
        Kitchen::UserError,
        "Verifier cinc_auditor does not support the wickedsauce Transport"
      )
    end

    it "does not dispatch to a method outside the build_ prefix" do
      allow(transport).to receive(:name).and_return("inspect")
