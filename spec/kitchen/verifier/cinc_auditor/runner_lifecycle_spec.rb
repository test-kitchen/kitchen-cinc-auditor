...
RSpec.describe Kitchen::Verifier::CincAuditor do
  ...
  describe "#runner_options" do
    ...
    it "leaves the runner defaults alone for options that are not configured" do
      config[:reporter] = nil

      options = verifier.send(:runner_options, transport)

      expect(options.keys).not_to include("reporter", "format", "output", "profiles_path")
    end

    it "defaults colour on when color is not configured" do
      expect(verifier.send(:runner_options, transport)).to include("color" => true)
    end

    it "templates output as well as reporters" do
      config[:output] = "/tmp/%{platform}/%{suite}.json"

      options = verifier.send(:runner_options, transport, {}, "almalinux-9", "inputs")

      expect(options).to include("output" => "/tmp/almalinux-9/inputs.json")
    end
  ...
  describe "#call" do
    ...
    it "survives a runner that reports no loaded profiles" do
      ensure_suite_directory("default")
      allow(runner).to receive(:add_target).and_return(nil)
