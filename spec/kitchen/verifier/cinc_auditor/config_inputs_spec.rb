...
RSpec.describe Kitchen::Verifier::CincAuditor do
  ...
  describe "#load_plugins" do
    ...
    let(:runtime) do
      instance_double(
        ...
        plugin_loader_class: loader_class,
    ...
    it "skips input caching on a runtime whose registry does not support it" do
      config[:cache_inputs] = true
      allow(runtime).to receive(:input_registry).and_return(double("registry without input caching"))

      expect { verifier.send(:load_plugins) }.not_to raise_error
    end
  end
  ...
  describe "#setup_plugin_config" do
    ...
    it "merges every configured plugin, not just the first" do
      audit_config = double("Cinc Auditor config")
      config[:plugin_config] = {
        "json" => { "enabled" => true },
        "html2" => { "alternate_css_file" => "custom.css" },
      }

      expect(audit_config).to receive(:merge_plugin_config).with("json", { "enabled" => true })
      expect(audit_config).to receive(:merge_plugin_config).with("html2", { "alternate_css_file" => "custom.css" })
